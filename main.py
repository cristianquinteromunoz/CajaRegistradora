...
class KuroApp(ctk.CTk):
    ...
    def aplicar_estilos_globales(self):
        ...
        style.configure("Treeview.Heading", background=theme.COLORS["table_header"],
                        foreground=theme.COLORS["text_main"], font=theme.FONTS["table_header"], borderwidth=0,
                        padding=5)
    ...
    def construir_interfaz_principal(self):
        ...
        ctk.CTkLabel(self.sidebar, text="KURO\nSYSTEMS", font=theme.FONTS["h2"],
                     text_color=theme.COLORS["text_main"]).pack(pady=(20, 20), padx=20)

        self.btn_alertas = ctk.CTkButton(self.sidebar, text="🔔 Alertas", font=theme.FONTS["body_bold"], height=40,
                                         command=self.mostrar_alertas)
        self.btn_alertas.pack(pady=(0, 20), padx=20, fill="x")

        if self.rol_actual == "admin":
            self.crear_btn("📊 Inteligencia Financiera", theme.COLORS["special"], theme.COLORS["special_hover"],
                           views.FinanzasView)
            self.crear_btn("📋 Inventario de Stock", theme.COLORS["primary"], theme.COLORS["primary_hover"],
                           views.InventarioView)
            self.crear_btn("➕ Ingresar Producto", theme.COLORS["primary"], theme.COLORS["primary_hover"],
                           views.FormularioProductoView)
            self.crear_btn("👥 Gestión Personal", theme.COLORS["bg_input"], theme.COLORS["bg_input"], views.UsuariosView)
            self.crear_btn("⚙️ Ajustes de Tienda", theme.COLORS["bg_input"], theme.COLORS["bg_input"],
                           views.AjustesView)

        self.crear_btn("🧾 Terminal de Cobro", theme.COLORS["primary"], theme.COLORS["primary_hover"], views.POSView)

        ctk.CTkButton(self.sidebar, text="🚪 Cerrar Sesión", font=theme.FONTS["body_bold"], height=40,
                      fg_color=theme.COLORS["danger"], hover_color=theme.COLORS["danger_hover"],
                      command=self.mostrar_login).pack(pady=(50, 10), padx=20, fill="x", side="bottom")
        ...
        if self.rol_actual == "admin":
            self.cambiar_vista(views.FinanzasView)
        else:
            self.cambiar_vista(views.POSView)

    def crear_btn(self, texto, color, hover, view_class):
        ctk.CTkButton(self.sidebar, text=texto, font=theme.FONTS["body"], height=40, anchor="w", fg_color=color,
                      hover_color=hover, command=lambda: self.cambiar_vista(view_class)).pack(pady=10, padx=20,
                                                                                              fill="x")

    # ==========================================
    # MOTOR DE ENRUTAMIENTO MODIFICADO (Caché POS)
    # ==========================================
    def cambiar_vista(self, view_class, **kwargs):
        if self.current_view:
            # Si salimos de la terminal, se OCULTA en lugar de destruirse para no borrar los carritos
            if isinstance(self.current_view, views.POSView):
                self.current_view.pack_forget()
            else:
                self.current_view.destroy()

        # Si solicitamos volver a la terminal de cobro
        if view_class == views.POSView:
            # La creamos solo si no existe en esta sesión
            if not hasattr(self, 'pos_cacheado') or not self.pos_cacheado.winfo_exists():
                self.pos_cacheado = view_class(self.main_container, self, **kwargs)

            self.current_view = self.pos_cacheado
            self.current_view.pack(fill="both", expand=True)
            self.current_view.enfocar_caja()  # Forzamos el foco en el escáner al abrirla
        else:
    ...
    def actualizar_campana_alertas(self):
        ...
        if total > 0:
            self.btn_alertas.configure(text=f"🔔 Alertas ({total})", fg_color=theme.COLORS["danger"],
                                       hover_color=theme.COLORS["danger_hover"])
        else:
            self.btn_alertas.configure(text="🔔 Alertas (0)", fg_color=theme.COLORS["success"],
    ...
    def mostrar_alertas(self):
        ...
        if not alertas_stk:
            ctk.CTkLabel(scroll_stk, text="Todos los productos tienen niveles óptimos.",
        ...
        if not alertas_cad:
            ctk.CTkLabel(scroll_cad, text="No hay productos próximos a vencer en los próximos 30 días.",
                         text_color=theme.COLORS["success"]).pack(pady=50)
        else:
